...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const uid = user.id;
      localStorage.removeItem(`studentConfig_${uid}`);
    ...
    <div className="page-enter flex items-center justify-center min-h-[70vh] px-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 max-w-md w-full border border-gray-100 dark:border-gray-700">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-gray-800 dark:text-white mt-3">Criar Conta</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Cadastre-se para começar a aprender</p>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          Já tem conta?{' '}
          <Link to="/login" className="text-blue-600 dark:text-blue-400 font-semibold hover:underline">Fazer Login</Link>
        </p>
